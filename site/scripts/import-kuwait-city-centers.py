#!/usr/bin/env python3
"""Build Kuwait's city-level catalog from a fixed GeoNames snapshot.

Usage:
  py -3.12 scripts/import-kuwait-city-centers.py <KW.txt> [output.csv]
"""
...
EXPECTED_GEONAMES_HASH = (
    "CF831E9E798DE436FEA21DE60D89AA0E1BB762244D05E901CDAA730267CBCA7B"
)
...
DEFAULT_OUTPUT = (
    SCRIPT_DIR.parent / "data" / "sources" / "kw-city-centers-2026-09-06.csv"
)
...
EXCLUDED_GEONAMES_IDS = {
    # Internal Kuwait City residential districts.
    "285856",  # Ad Dasmah
    "285603",  # Janub as Surrah
    # Agricultural farm locality rather than a city.
    "11579278",  # Mazari' al Abdali
}
...
def load_places(path: Path) -> list[list[str]]:
    ...
    if actual_hash != EXPECTED_GEONAMES_HASH:
        raise SystemExit(f"GeoNames 科威特快照哈希变化：{actual_hash}")
...
def build_rows(places: list[list[str]]) -> list[dict[str, str]]:
    ...
    for place in places:
        ...
        if not (46.5 <= longitude <= 48.7 and 28.4 <= latitude <= 30.2):
            raise SystemExit(f"{source_name}（{geonames_id}）中心点超出科威特范围")
    ...
    if dict(sorted(feature_counts.items())) != EXPECTED_FEATURE_COUNTS:
        raise SystemExit(f"科威特城市层级数量变化：{dict(feature_counts)}")
    if len(rows) != 14:
        raise SystemExit(f"科威特城市数量变化：{len(rows)}")
    if set(ADMIN_AREA_NAMES.values()) != {row["admin_area"] for row in rows}:
        raise SystemExit("科威特省级覆盖发生变化")
...
def main() -> None:
    if len(sys.argv) not in {2, 3}:
        raise SystemExit(
            "用法：py -3.12 scripts/import-kuwait-city-centers.py <KW.txt> [output.csv]"
        )
