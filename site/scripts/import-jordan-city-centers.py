#!/usr/bin/env python3
"""Build Jordan's city-level catalog from a fixed GeoNames snapshot.

Usage:
  py -3.12 scripts/import-jordan-city-centers.py <JO.txt> [output.csv]
"""
...
EXPECTED_GEONAMES_HASH = (
    "CDCC54B0F5741106D1053DECA5FEC8917B70991A6A679D66073A1C1DB2EA67E7"
)
...
DEFAULT_OUTPUT = (
    SCRIPT_DIR.parent / "data" / "sources" / "jo-city-centers-2026-09-06.csv"
)
...
EXCLUDED_GEONAMES_IDS = {
    # Non-city objects.
    "11428685",  # Rukban refugee camp
    "10861483",  # Al al-Bayt University junction
    "10640344",  # Sabha junction; retain Sabha (247176)
    "250094",  # Azraq junction
    # Internal Greater Amman districts and neighborhoods.
    "13286467",  # Suwaylih
    "13308287",  # Marj Al Hamam
    "248460",  # Khuraybat as Suq
    "246314",  # Umm as Summaq
    "246013",  # Wadi as Sir
    "250637",  # Al Jubayhah
    "10862121",  # Al Abdalli
    "250628",  # Al Juwayyidah
    # Duplicate city records; retain the administrative-seat records.
    "11352362",  # Ruwaished -> Ar Ruwayshid (250317)
    "7838895",  # Russeifa -> Ar Rusayfah (250321)
    "250624",  # Qir Moav -> Karak City (6946409)
}
...
def load_places(path: Path) -> list[list[str]]:
    ...
    if actual_hash != EXPECTED_GEONAMES_HASH:
        raise SystemExit(f"GeoNames 约旦快照哈希变化：{actual_hash}")
...
def build_rows(places: list[list[str]]) -> list[dict[str, str]]:
    ...
    for place in places:
        ...
        if not (34.8 <= longitude <= 39.4 and 29.0 <= latitude <= 33.5):
            raise SystemExit(f"{source_name}（{geonames_id}）中心点超出约旦范围")
    ...
    if dict(sorted(feature_counts.items())) != EXPECTED_FEATURE_COUNTS:
        raise SystemExit(f"约旦城市层级数量变化：{dict(feature_counts)}")
    if len(rows) != 115:
        raise SystemExit(f"约旦城市数量变化：{len(rows)}")
    if set(ADMIN_AREA_NAMES.values()) != {row["admin_area"] for row in rows}:
        raise SystemExit("约旦省级覆盖发生变化")
...
def main() -> None:
    if len(sys.argv) not in {2, 3}:
        raise SystemExit(
            "用法：py -3.12 scripts/import-jordan-city-centers.py <JO.txt> [output.csv]"
        )
