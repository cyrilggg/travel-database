#!/usr/bin/env python3
"""Build Qatar's city-level catalog from a fixed GeoNames snapshot.

Usage:
  py -3.12 scripts/import-qatar-city-centers.py <QA.txt> [output.csv]
"""
...
EXPECTED_GEONAMES_HASH = (
    "393AE7831C80D9C3EA2FC4022187956D9B05DAD5256288F5BEE1C8FD93A88E04"
)
...
DEFAULT_OUTPUT = (
    SCRIPT_DIR.parent / "data" / "sources" / "qa-city-centers-2026-09-06.csv"
)
...
EXCLUDED_GEONAMES_IDS = {
    # Doha and Al Rayyan internal districts, not separate cities.
    "289698",  # Nuayjah
    "289716",  # Muaydhir / Rawdat Rashid
    "289545",  # Umm Ghuwaylinah
}
...
def load_places(path: Path) -> list[list[str]]:
    ...
    if actual_hash != EXPECTED_GEONAMES_HASH:
        raise SystemExit(f"GeoNames 卡塔尔快照哈希变化：{actual_hash}")
...
def build_rows(places: list[list[str]]) -> list[dict[str, str]]:
    ...
    for place in places:
        ...
        if admin_area is None:
            raise SystemExit(
                f"{source_name}（{geonames_id}）的市代码未收录：{place[10]}"
            )

        latitude = float(place[4])
        longitude = float(place[5])
        if not (50.7 <= longitude <= 51.8 and 24.4 <= latitude <= 26.3):
            raise SystemExit(f"{source_name}（{geonames_id}）中心点超出卡塔尔范围")
    ...
    if dict(sorted(feature_counts.items())) != EXPECTED_FEATURE_COUNTS:
        raise SystemExit(f"卡塔尔城市层级数量变化：{dict(feature_counts)}")
    if len(rows) != 13:
        raise SystemExit(f"卡塔尔城市数量变化：{len(rows)}")
    if set(ADMIN_AREA_NAMES.values()) != {row["admin_area"] for row in rows}:
        raise SystemExit("卡塔尔市级覆盖发生变化")
...
def main() -> None:
    if len(sys.argv) not in {2, 3}:
        raise SystemExit(
            "用法：py -3.12 scripts/import-qatar-city-centers.py <QA.txt> [output.csv]"
        )
